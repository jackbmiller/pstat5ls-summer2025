...
```{r tryIt1, error = T}
# Run this code chunk to create a scatterplot with bill_length_mm as the 
...
```{r tryIt3, error = T}
# Run this code chunk to subset the gentoo data to contain only numeric variables. 
# We will call this subset numericGentoo
...
```{r tryIt5, error = T}
# Run this code chunk to (1) create the linear regression model for body mass on
# flipper length, and assign it the name line1, then (2) summarize the linear 
# regression model called line1
...
```{r tryIt6, error = T}
# Run this code chunk to create a scatterplot of body mass on flipper length,
